...
EXTISM_LL_API void __extism_load(const ExtismPointer offs, void *dest,
  ...
  uint64_t *i64_buffer = (uint64_t *)dest;
  ...
  for (uint8_t *u8_buffer = (uint8_t *)dest; remainder_offset < remainder_end;
       remainder_offset++) {
...
EXTISM_LL_API void __extism_store(ExtismPointer offs, const void *buffer,
  ...
  const uint64_t *i64_buffer = (const uint64_t *)buffer;
  for (size_t chunk_idx = 0; chunk_idx < chunk_count; chunk_idx++) {
  ...
  for (const uint8_t *u8_buffer = (const uint8_t *)buffer; remainder_offset < remainder_end;
       remainder_offset++) {
